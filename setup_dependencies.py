...
║  Versão: 2.2 — Julho 2026 (+ Octave + Dynare)              ║
║                                                            ║
║  ★ CORREÇÃO CRÍTICA v2.2: config.json limpo                ║
║    Removeu-se chaves inválidas (tab_title, model)          ║
║    que OpenCode rejeitava — agora padrão PesquisAI         ║
╚══════════════════════════════════════════════════════════════╝
...
*AgenteDSGE · v2.2 · Protocolo de 17 etapas · Octave + Dynare · Zero Alucinação · Reprodutibilidade Absoluta · Transparência Total*
"""


# ══════════════════════════════════════════════════════════════
# 4. CONFIGURAÇÃO DO AGENTE NO OPENCODE
# ══════════════════════════════════════════════════════════════
# ★ CORREÇÃO v2.2: Config.json limpo — igual ao padrão PesquisAI
#
# ANTES (v2.1 — causava erro):
#   cfg["tab_title"] = "AgenteDSGE — Modelagem Macroeconômica"  ← INVALIDO
#   cfg["model"] = "deepseek-v4-flash-free"                     ← INVALIDO
#
# DEPOIS (v2.2 — corrigido):
#   cfg["default_agent"] = "dsge"  ← UNICA chave valida
#
# O OpenCode reconhece apenas as chaves documentadas no schema:
#   - default_agent  ✅
#   - tab_title      ❌ (rejeitado = Configuration is invalid)
#   - model          ❌ (rejeitado = Configuration is invalid)
# ══════════════════════════════════════════════════════════════
...
def setup_agent():
    ...
    ATENÇÃO: Apenas chaves válidas do schema do OpenCode são escritas.
    Nunca adicionar 'tab_title' ou 'model' — causam erro de validação.
    """
    ...
    # ── Lê config.json existente ou cria vazio ──
    try:
    ...
    # ── Remove chaves inválidas que o OpenCode NÃO reconhece ──
    # Isso limpa qualquer resíduo de versões anteriores do AgenteDSGE
    CHAVES_INVALIDAS = {"tab_title", "model"}
    chaves_removidas = [k for k in CHAVES_INVALIDAS if k in cfg]
    for k in chaves_removidas:
        del cfg[k]
        log_warn(f"Chave inválida removida do config.json: '{k}'")

    # ── Define APENAS chaves válidas ──
    # O schema do OpenCode aceita apenas: default_agent
    cfg["default_agent"] = "dsge"

    # ⛔ NÃO adicionar:
    #   - cfg["tab_title"] = "..."  ← OpenCode rejeita
    #   - cfg["model"] = "..."      ← OpenCode rejeita
    # O modelo/LLM é definido por variável de ambiente ou CLI, não no config.json
    ...
    log_ok(f"Config.json limpo — sem chaves inválidas (tab_title, model removidos)")
...
def verify_installation():
    ...
    # 7. Config.json válido (VERIFICAÇÃO EXTRA — garante que não tem chave inválida)
    try:
        with open(OPENCODE_CFG, "r", encoding="utf-8") as f:
            cfg = json.load(f)
        config_valido = not any(k in cfg for k in ["tab_title", "model"])
        if not config_valido:
            log_warn("config.json ainda contém chaves inválidas! Executando limpeza...")
            for k in ["tab_title", "model"]:
                cfg.pop(k, None)
            with open(OPENCODE_CFG, "w", encoding="utf-8") as f:
                json.dump(cfg, f, indent=2)
            log_ok("config.json limpo automaticamente.")
            config_valido = True
        checks.append(("Config.json válido", config_valido))
    except Exception:
        checks.append(("Config.json válido", False))
        all_ok = False
...
def run_all(drive_base_path=None):
    ...
    print(f"\n")
    print(f"  ╔══════════════════════════════════════════════════╗")
    print(f"  ║     🔷 CONFIGURAÇÃO DA AGENTEDSGE v2.2        ║")
    print(f"  ║     Modelagem DSGE — Protocolo de 17 Etapas    ║")
    print(f"  ║     Config.json limpo — sem chaves inválidas   ║")
    print(f"  ╚══════════════════════════════════════════════════╝")
    print(f"")
    ...
    # ── 4. Configurar agente (system prompt + config.json LIMPO) ──
    print(f"\n  ┌{'─'*50}┐")
    print(f"  │ 🧠 FASE 4: System Prompt + Config.json         │")
    print(f"  └{'─'*50}┘")
    setup_agent()
    ...
    if all_ok:
        ...
        print(f"  ██   Config.json: válido (sem chaves inválidas)██")
    ...
    try:
        ...
        if "tab_title" in cfg_final or "model" in cfg_final:
            log_warn("⚠️  AVISO: config.json AINDA contém chaves inválidas!")
            log_warn("   Execute manualmente para limpar:")
            log_warn(f"   python3 -c \"import json; f=open('{OPENCODE_CFG}','r+'); cfg=json.load(f); cfg.pop('tab_title',None); cfg.pop('model',None); f.seek(0); json.dump(cfg,f,indent=2); f.truncate()\"")
        else:
            log_ok("Config.json final verificado: 100% válido ✅")
    except Exception:
        pass
